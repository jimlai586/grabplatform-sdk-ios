...
class GrabApi {
    ...
    static func fetchServiceConfigurations(session: URLSession, serviceDiscoveryUrl: String, completion: @escaping (ServiceEndPoints, GrabIdPartnerError?) -> Void) {
        ...
        guard let url = createUrl(baseUrl: serviceDiscoveryUrl) else {
            let error = GrabIdPartnerError(code: .invalidUrl,
                    localizedMessage: serviceDiscoveryUrl,
    ...
    static func fetchToken(exchangeTokenEndPoint: String, clientID: String = "", code: String = "", codeVerifier: String = "",
...
//        let session = URLSession.shared
                let task = URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
                    if let error = error {
                        let error = GrabIdPartnerError(code: .serviceError,
                                localizedMessage: Loc.serviceError,
    ...
    static func fetchGetIdTokenInfo(getIdTokenInfoEndpoint: String, clientId: String = "", idToken: String = "", nonce: String = "",
                                    completion: @escaping ([String: Any]?, GrabIdPartnerError?) -> Void) {
        // Assemble query params to pass to createUrlWithParams
        let paramValues = [
            "client_id": clientId,
...
//        let session = URLSession.shared
    ...
    static func fetchGrabAppDeeplinks(customProtocolUrl: String, completion: @escaping ([[String: String]], GrabIdPartnerError?) -> Void) {
        guard let url = createUrl(baseUrl: customProtocolUrl) else {
            let error = GrabIdPartnerError(code: .invalidUrl,
                    localizedMessage: Loc.invalidCustomProtocolUrl,
                ...
                do {
                    ...
                    if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                       let customProtocols = json["custom_protocols"] as? [String] {
                        let deeplinkProtocolInfos = customProtocols.compactMap {
                            $0.data(using: String.Encoding.utf8)
                        }
                        if deeplinkProtocolInfos.count > 0 {
                            grabAppDeeplinkMappings = try deeplinkProtocolInfos.compactMap {
